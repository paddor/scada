...
describe Scada::StatusCode do
  describe "#good?" do
    it "returns true for GOOD status" do
      status = Scada::StatusCode.new(code: 0x00000000)
      assert status.good?
    end

    it "returns false for BAD status" do
      status = Scada::StatusCode.new(code: 0x80000000)
      refute status.good?
    end
  end
  ...
  describe "#bad?" do
    it "returns true for BAD status" do
      status = Scada::StatusCode.new(code: 0x80010000)
      assert status.bad?
    end
  ...
  describe "#uncertain?" do
    it "returns true for uncertain status" do
      status = Scada::StatusCode.new(code: 0x40000000)
  ...
  describe "#inspect" do
    it "includes class name and hex code" do
      status = Scada::StatusCode.new(code: 0x00000000)
      assert_includes status.inspect, "Scada::StatusCode"
