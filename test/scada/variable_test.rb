...
describe "Variables" do
  ...
  it "round-trips a SByte (int8)" do
    with_server do |server, port|
      server.add_variable("ns=1;s=sbyte", type: :sbyte, value: -42,
                          display_name: "SByte")
      Sync do |task|
        server_task = task.async { server.run }
        client = new_client("opc.tcp://localhost:#{port}")
  ...
  it "round-trips a Byte (uint8)" do
    with_server do |server, port|
      server.add_variable("ns=1;s=byte", type: :byte, value: 200,
                          display_name: "Byte")
      Sync do |task|
        server_task = task.async { server.run }
        client = new_client("opc.tcp://localhost:#{port}")
  ...
  it "round-trips an Int16" do
    with_server do |server, port|
      server.add_variable("ns=1;s=i16", type: :int16, value: -1000,
                          display_name: "Int16")
      Sync do |task|
        server_task = task.async { server.run }
        client = new_client("opc.tcp://localhost:#{port}")
  ...
  it "round-trips a UInt16" do
    with_server do |server, port|
      server.add_variable("ns=1;s=u16", type: :uint16, value: 65535,
                          display_name: "UInt16")
      Sync do |task|
        server_task = task.async { server.run }
        client = new_client("opc.tcp://localhost:#{port}")
  ...
  it "round-trips a UInt32" do
    with_server do |server, port|
      server.add_variable("ns=1;s=u32", type: :uint32, value: 3_000_000_000,
                          display_name: "UInt32")
      Sync do |task|
        server_task = task.async { server.run }
        client = new_client("opc.tcp://localhost:#{port}")
  ...
  it "round-trips an Int64" do
    with_server do |server, port|
      server.add_variable("ns=1;s=i64", type: :int64, value: -9_000_000_000,
                          display_name: "Int64")
      Sync do |task|
        server_task = task.async { server.run }
        client = new_client("opc.tcp://localhost:#{port}")
  ...
  it "round-trips a UInt64" do
    with_server do |server, port|
      server.add_variable("ns=1;s=u64", type: :uint64, value: 18_000_000_000_000_000_000,
                          display_name: "UInt64")
      Sync do |task|
        server_task = task.async { server.run }
        client = new_client("opc.tcp://localhost:#{port}")
  ...
  it "round-trips a Double (64-bit float)" do
    with_server do |server, port|
      server.add_variable("ns=1;s=dbl", type: :double, value: 1.23456789012345e+200,
                          display_name: "Double")
      Sync do |task|
        server_task = task.async { server.run }
        client = new_client("opc.tcp://localhost:#{port}")
  ...
  it "round-trips a Float (32-bit)" do
    with_server do |server, port|
      server.add_variable("ns=1;s=flt", type: :float, value: 3.14,
                          display_name: "Float")
      Sync do |task|
        server_task = task.async { server.run }
        client = new_client("opc.tcp://localhost:#{port}")
  ...
  it "round-trips a DateTime" do
    with_server do |server, port|
      now = Time.utc(2025, 6, 15, 12, 30, 45)
      server.add_variable("ns=1;s=ts", type: :datetime, value: now,
  ...
  it "round-trips a ByteString" do
    with_server do |server, port|
      binary = "\x00\x01\xFF\xFE".b
      server.add_variable("ns=1;s=blob", type: :byte_string, value: binary,
                          display_name: "Blob")
      Sync do |task|
        server_task = task.async { server.run }
        client = new_client("opc.tcp://localhost:#{port}")
  ...
  it "reads an array of doubles" do
    with_server do |server, port|
      server.add_variable("ns=1;s=temps", type: :double, value: [20.0, 21.5, 22.0],
  ...
  it "round-trips an array of int32" do
    with_server do |server, port|
      server.add_variable("ns=1;s=ids", type: :int32, value: [1, 2, 3],
                          display_name: "IDs")
      Sync do |task|
        server_task = task.async { server.run }
        client = new_client("opc.tcp://localhost:#{port}")
  ...
  it "round-trips an array of strings" do
    with_server do |server, port|
      server.add_variable("ns=1;s=tags", type: :string, value: ["a", "b"],
                          display_name: "Tags")
      Sync do |task|
        server_task = task.async { server.run }
        client = new_client("opc.tcp://localhost:#{port}")
  ...
  it "round-trips an array of booleans" do
    with_server do |server, port|
      server.add_variable("ns=1;s=bits", type: :boolean, value: [true, false, true],
                          display_name: "Bits")
      Sync do |task|
        server_task = task.async { server.run }
        client = new_client("opc.tcp://localhost:#{port}")
